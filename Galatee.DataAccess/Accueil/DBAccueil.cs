...
namespace Galatee.DataAccess
{
    public class DBAccueil
    {
        ...
        /* 19/01/2021 Optimisation du classeur client Regroupe */
        public int RechercherNombreClientRegrouper(CsRegCli leRegroupement)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    Resultat = reader["NOMBRECLIENTREGROUPEMENT"].ToString();
                }
                reader.Close(); 
        ...
        public Dictionary<List<CsClientRechercher>, List<CsLclient>> RechercherClientRegrouper(CsRegCli leRegroupement, int Indice, int Nombre)
        {
            ...
            try
            {
                ...
                List<CsClientRechercher> LesClient = RetourneClientSpx(leRegroupement, Indice, Nombre);
                List<CsLclient> lesFacture = new List<CsLclient>();
                foreach (CsClientRechercher item in LesClient)
                    lesFacture.AddRange( RetourneCompteClientSpx(item, string.Empty, string.Empty));
        ...
        public List<CsLclient> RechercherFactureRegrouper(CsClientRechercher leClient)
        {
            //cmd.CommandText = "SPX_GUI_RECHERCHE_CLIENT";
            try
            {
                return RetourneCompteClientSpx(leClient,null,null);
            }
            catch (Exception ex)
            {
                throw ex;
...
*/
       ...
       public string RetourneOrdreMax(int fk_idcentre, string centre, string client, string produit)
       {
        ...
        #region MAJ Modification demande
            ...
            #endregion
            #region MyRegion
            public List<CsClientRechercher> RetourneClientSpx(CsRegCli leClient, int Indice, int Nombre)
                {
                    cn = new SqlConnection(ConnectionString);
                ...
                public List<CsLclient> RetourneCompteClientSpx(CsClientRechercher  leClient, string periodeDebut, string PeriodeFin)
                {
                    cn = new SqlConnection(ConnectionString);
                    ...
                    cmd.Parameters.Add("@FK_IDCLIENT", SqlDbType.Int).Value = leClient.PK_ID;
...
*/
        ...
        */
        ...
        public void InsertDCanalisation(CsCanalisation laCan, SqlCommand cmds)
        {
            cmds.CommandTimeout = 3000;
            cmds.CommandType = CommandType.StoredProcedure;
            cmds.CommandText = "SPX_ACC_INSERT_DCANALISATION";
            if (cmds.Parameters != null && cmds.Parameters.Count != 0) cmds.Parameters.Clear();
            ...
            try
            {
                ...
                if (laDemande.LaDemande.TYPEDEMANDE == Enumere.ChangementProduit)
                {
                    ...
                    */
                    ...
                    CsEvenement evePose = laDemande.LstEvenement.FirstOrDefault(y => y.CAS == Enumere.CasPoseCompteur);
                    CsEvenement eveDepose = laDemande.LstEvenement.FirstOrDefault(y => y.CAS == Enumere.CasDeposeCompteur);
                    ...
                    // ZEG 03/01/2021 List<CsCanalisation> lcanal = Select_CannalisationCompteur(laDemande.LaDemande.FK_IDCENTRE, laDemande.LaDemande.CENTRE, laDemande.LaDemande.CLIENT, laDemande.LaDemande.PRODUIT, laConnection);
                    List<CsCanalisation> lcanal = Select_CannalisationCompteur(laDemande.LaDemande.FK_IDCENTRE, laDemande.LaDemande.CENTRE, laDemande.LaDemande.CLIENT, null /*ZEG 03/01/2021*/, laConnection);
                    ...
                    if (lcanal2 != null && lcanal2.Count > 0)
                    {
                        ...
                        if (eveDepose != null)
                            canal.DEPOSE = eveDepose.DATEEVT;
                        else
                    ...
                    if (evePose != null)
                        laDemande.Abonne.DABONNEMENT = evePose.DATEEVT;
                    else
                        laDemande.Abonne.DABONNEMENT = System.DateTime.Today;
                    /** ZEG **/
                    ...
                    if (laDemande.LstEvenement != null && laDemande.LstEvenement.Count != 0)
                    {
                        ...
                        if (lst != null && lst.Count != 0)
                        {
                            ...
                            {
                                item.LOTRI = laDemande.LaDemande.CENTRE + "00004";
                ...
                #endregion
                #region ChangementCompteur/Etalonnage
                if (laDemande.LaDemande.TYPEDEMANDE == Enumere.ChangementCompteur
                    || laDemande.LaDemande.TYPEDEMANDE == Enumere.Etalonage) //ZEG 20/01/2021
                {
                    CsBrt brt = Select_brt(laDemande.LaDemande.FK_IDCENTRE, laDemande.LaDemande.CENTRE, laDemande.LaDemande.CLIENT, laConnection);
                    CsBrt dbrt = Select_Dbrt(laDemande.LaDemande.NUMDEM, laDemande.LaDemande.PK_ID, laConnection);
                    ...
                    if (laDemande.LstCanalistion != null)
                        foreach (CsCanalisation item in laDemande.LstCanalistion)
                        {
                            ...
                            if (laDemande.LaDemande.TYPEDEMANDE == Enumere.Etalonage) //ZEG 20/01/2021
                                InsertDCanalisation(item, laCommande); 

                            InsertOrUpdateCANALISATION(item, laCommande);

                            CsEvenement leEvt = laDemande.LstEvenement.FirstOrDefault(t => t.CAS == Enumere.CasPoseCompteur && t.POINT == item.POINT);
                            leEvt.FK_IDCANALISATION = item.PK_ID;
                            leEvt.FK_IDABON = laDemande.Abonne.PK_ID;
                            leEvt.FK_IDCOMPTEUR = IdCompteur;
                            leEvt.STATUS = Enumere.EvenementPurger;
        ...
        public CsDemande ChargerDetailClient(int fk_idcentre, string centre, string client, string ordre, string produit, string Typedemande)
        {
            ...
            try
            {
                ...
                if (lstCan != null && lstCan.Count != 0)
                {
                    ...
                    /* ZEG 26/01/2021 => CAS MODIFICATION COMPTEUR*/
                        //_LaDemande.LstCanalistion.Add(lstCan.OrderByDescending(p => p.POSE).First());

                    if (Typedemande == Enumere.ModificationCompteur)
                    {
                        if ((lstCan.First().PRODUIT != Enumere.ElectriciteMT && lstCan.Count > 1) || (lstCan.First().PRODUIT == Enumere.ElectriciteMT && lstCan.Count > 6))
                            _LaDemande.LstCanalistion.AddRange(lstCan.Where(a => a.DEPOSE == null).OrderByDescending(p => p.POSE).ToList());
                        else
                            _LaDemande.LstCanalistion.AddRange(lstCan.OrderByDescending(p => p.POSE).ToList());
                    }
                    else
                        _LaDemande.LstCanalistion.Add(lstCan.OrderByDescending(p => p.POSE).First());
                    /* Fin ZEG*/
        ...
        */
        ...
        public CsDemande ChargerDetailDemandeConsultation(int idDemande, string NumDemande)
        {
            ...
            try
            {
                ...
                if (_LaDemande.LaDemande != null && !string.IsNullOrEmpty(_LaDemande.LaDemande.TYPEDEMANDE))
                {
                    if (_LaDemande.LaDemande.TYPEDEMANDE != Enumere.DemandeReclamation)
                    {
                    ...
                    /* Prendre en compte la consultation des demande de reclamation 20/01/2021 */
                       else  if (_LaDemande.LaDemande.TYPEDEMANDE == Enumere.DemandeReclamation)
                        {
                            _LaDemande.InfoDemande = new DB_WORKFLOW().RecupererInfoDemandeParCodeTDem(_LaDemande.LaDemande, laConnection);
                            CsClient lesClient = Select_client(_LaDemande.LaDemande.FK_IDCENTRE, _LaDemande.LaDemande.CENTRE, _LaDemande.LaDemande.CLIENT, _LaDemande.LaDemande.ORDRE, laConnection);
                            if (lesClient != null )
                            {
                                _LaDemande.LeClient = lesClient;
                                _LaDemande.LaDemande.ORDRE = lesClient.ORDRE;
                                _LaDemande.SocietePrives = Select_SocietePrive(_LaDemande.LeClient.PK_ID , laConnection);
                                _LaDemande.AdministrationInstitut = Select_Administration_Institut(_LaDemande.LeClient.PK_ID, laConnection);
                                _LaDemande.PersonePhysique = Select_PersonnePhysique(_LaDemande.LeClient.PK_ID, laConnection);

                                _LaDemande.Ag = Select_Ag(_LaDemande.LeClient.FK_IDCENTRE.Value,_LaDemande.LeClient.CENTRE, _LaDemande.LeClient.REFCLIENT, laConnection);
                                _LaDemande.Abonne = Select_Abon(_LaDemande.LeClient.FK_IDCENTRE.Value , _LaDemande.LeClient.CENTRE, _LaDemande.LeClient.REFCLIENT, _LaDemande.LeClient.ORDRE, _LaDemande.LeClient.PRODUIT, laConnection);
                                _LaDemande.Branchement = Select_brt(_LaDemande.LeClient.FK_IDCENTRE.Value,_LaDemande.LeClient.CENTRE ,_LaDemande.LeClient.REFCLIENT , laConnection);
                            }
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (_LaDemande.LaDemande.TYPEDEMANDE == Enumere.BranchementAbonement ||
                                ...
                                _LaDemande.LaDemande.TYPEDEMANDE == Enumere.ChangementCompteur ||
