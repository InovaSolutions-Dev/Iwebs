...
namespace WcfService
{
        ...
        public  List<CsForfait> ChargerForfait()
    {
        ...
        public int RechercherNombreClientRegrouper(CsRegCli leRegroupement)
        {
            ...
            {
                DBAccueil db = new DBAccueil();
                return db.RechercherNombreClientRegrouper(leRegroupement);
            }
            catch (Exception ex)
            {
                ErrorManager.WriteInLogFile(this, ex.Message);
        ...
        public Dictionary<List<CsClientRechercher>, List<CsLclient>> RechercherClientRegrouper(CsRegCli leRegroupement, int Indice, int Nombre, ref bool Sortie)
        {
            try
            {
        ...
        public List<CsLclient> RechercherFactureRegrouper(CsClientRechercher  leClient)
        {
            ...
            {
                //LKO le 06/01/2021 trace des duré d'execution//
                ErrorManager.WriteInLogFile(this, "Debut rechercher de donnée facture ");
                DBAccueil db = new DBAccueil();
                List<CsLclient> lstResultat = db.RechercherFactureRegrouper(leClient);
                ErrorManager.WriteInLogFile(this, "Fin rechercher de donnée facture ");
                return lstResultat;
                //return db.RechercherClientRegrouper(leRegroupement);
                //Fin  LKO
            }
        ...
        public List<CsDetailAffectationScelle> RetourneBorneDetailLotScelle(int IdDemande)
        {

            try
            {
                DBScelle db = new DBScelle();
                List<CsDetailAffectationScelle> lstDetail = db.RetourneListeDetailAffectationScelle(IdDemande);
                CsDetailAffectationScelle resultSupp = lstDetail.Where(u=>u.EstLivre == false).First(x => x.Nuemro_Scelle == lstDetail.Max(y => y.Nuemro_Scelle));
                CsDetailAffectationScelle resultInf = lstDetail.Where(u => u.EstLivre == false).First(x => x.Nuemro_Scelle == lstDetail.Min(y => y.Nuemro_Scelle));
                List<CsDetailAffectationScelle> Resultat = new List<CsDetailAffectationScelle>();
                resultSupp.NumScelleDepart = resultInf.Nuemro_Scelle;
                resultSupp.NumScelleFin = resultSupp.Nuemro_Scelle;
                Resultat.Add(resultSupp);
                return Resultat;
